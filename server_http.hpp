...
namespace SimpleWeb {
    ...
    public:
      ...
      void send(const std::function<void(const error_code &)> &callback = nullptr) {
        ...
        asio::async_write(*session->connection->socket, streambuf, [self, callback](const error_code &ec, size_t /*bytes_transferred*/) {
          ...
          auto lock = self->session->connection->cancel_handlers->lock();
          if(!lock)
            return;
          if(callback)
            callback(ec);
  ...
  protected:
    ...
    public:
      ...
      Connection(std::shared_ptr<CancelHandlers> cancel_handlers, Args &&... args) : cancel_handlers(std::move(cancel_handlers)), socket(new socket_type(std::forward<Args>(args)...)) {}

      std::shared_ptr<CancelHandlers> cancel_handlers;
  ...
  protected:
    ...
    ServerBase(unsigned short port) : config(port), connections(new std::unordered_set<Connection *>()), connections_mutex(new std::mutex()), cancel_handlers(new CancelHandlers()) {}
    ...
    std::shared_ptr<Connection> create_connection(Args &&... args) {
      ...
      auto connection = std::shared_ptr<Connection>(new Connection(cancel_handlers, std::forward<Args>(args)...), [connections, connections_mutex](Connection *connection) {
        {
          std::unique_lock<std::mutex> lock(*connections_mutex);
    ...
    void read_request_and_content(const std::shared_ptr<Session> &session) {
      ...
      asio::async_read_until(*session->connection->socket, session->request->streambuf, "\r\n\r\n", [this, session](const error_code &ec, size_t bytes_transferred) {
        ...
        auto lock = session->connection->cancel_handlers->lock();
        if(!lock)
          return;
        if(!ec) {
