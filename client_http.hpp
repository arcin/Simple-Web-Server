...
namespace SimpleWeb {
  ...
  protected:
    ...
    public:
      ...
      Connection(std::shared_ptr<CancelHandlers> cancel_handlers, long timeout, Args &&... args)
          : cancel_handlers(std::move(cancel_handlers)), timeout(timeout), socket(new socket_type(std::forward<Args>(args)...)) {}
  ...
  protected:
    ...
    ClientBase(const std::string &host_port, unsigned short default_port) : cancel_handlers(new CancelHandlers()) {
  ...
  protected:
    std::shared_ptr<Connection> create_connection() override {
      return std::make_shared<Connection>(cancel_handlers, config.timeout, *io_service);
    }
